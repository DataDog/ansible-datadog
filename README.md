...
| `datadog_agent_major_version`                                                                                                                         | The major version of the Agent which will be installed. Its possible values are 5, 6, 7. If not set, then `datadog_agent_version` must be set. If both this variable and `datadog_agent_version` are set, they must be compatible (the major version in `datadog_agent_version` should be `datadog_agent_major_version`). |
| `datadog_agent_version`                                                                                                                         | The pinned version of the Agent to install (optional, but highly recommended). Examples: `1:6.0.0-1` on apt-based platforms, `6.0.0-1` on yum-based platforms, `6.0.0` on Windows platforms. If not set, then `datadog_agent_major_version` must be set. If both this variable and `datadog_agent_major_version` are set, they must be compatible (the major version in `datadog_agent_version` should be `datadog_agent_major_version`). **Note** Downgrades are not supported on Windows platforms. |
