...
| `datadog_apm_instrumentation_lib_versions`     | List of APM libraries and versions to install if `host` or `docker` injection is enable. Example: `["java:1.23.0", "python:2.1.7"]
If `latest` is desired, use `datadog_apm_instrumentation_languages` to specify the languages needed. `["all"]` is not
allowed here. |
