...
If you were previously using the Agent 5 variables `datadog_agent5_apt_repo`, `datadog_agent5_yum_repo`, or `datadog_agent5_zypper_repo` to set custom Agent 5 repositories, use `datadog_apt_repo`, `datadog_yum_repo`, or `datadog_zypper_repo`(with `datadog_agent_major_version` set to `5` or `datadog_agent_version` pinned to a specific version of agent 5) instead.
...
| `datadog_agent_version`                                                                                                                         | The pinned version of the Agent to install (optional, but highly recommended). Example: `7.15.0`. Using `datadog_agent_major_version` is not needed is `datadog_agent_version` is used.. **Note** Downgrades are not supported on Windows platforms. |
